	...
	public partial class DCodeCompletionProvider
	{
		...
		public static void AddAllClassMembers(DataType selectedExpression, ref List<ICompletionData> rl, bool all, ImageList icons)
		{
			if (selectedExpression != null)
			{
				...
				if ((selectedExpression as DataType).superClass == "")
				{
					if (!DTokens.ClassLike[(selectedExpression as DataType).TypeToken] && selectedExpression.Parent != null && (selectedExpression.Parent as DataType).fieldtype != FieldType.Root)
						AddAllClassMembers(selectedExpression.Parent, ref rl, all, icons);
					return;
				}
