...
namespace D_IDE
{
    public partial class StartPage : DockContent
    {
        ...
        /// <summary>
        /// Creates a seperate thread for news retrieval due to bug in DownloadStringAsync
        /// </summary>
        public void UpdateNews()
        {
            if (!webclient.IsBusy && !downloading)
            {
                downloading = true;
        ...
        private void ParseNews(string result)
        {
            ...
            {
                nl.Clear();
                XmlTextReader xtr = new XmlTextReader(new StringReader(result));
